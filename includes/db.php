...
function init_schema(PDO $pdo): void {
    ...
    // Rebrand VitaTrack → Thrive: fix brand name in already-seeded lesson copy.
    $pdo->exec("UPDATE lessons SET body = REPLACE(body, 'VitaTrack', 'Thrive') WHERE body LIKE '%VitaTrack%'");
...
function seed_lessons(PDO $pdo): void {
    ...
    $L = [
        ...
        ['⚖️','Habits','The daily weigh-in ritual',
         "Weigh daily, same conditions: morning, after the bathroom, before food or water, minimal clothes. Log it in Thrive even when it's ugly — especially when it's ugly, because skipping bad-news weigh-ins is how tracking dies. Daily weighers lose more and keep it off better in studies, but only when they've internalized lesson five: the number is noise, the trend is signal. Your Progress chart smooths the noise for you. One more thing: never weigh after a restaurant night and panic — you know about sodium now. Data without drama."],
